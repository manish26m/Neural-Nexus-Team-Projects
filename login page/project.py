...
def reset_password():
    ...
    if entered_otp == generated_otp:
        ...
        reset_label1=tb.Label(new_centered_frame,text="Enter new password:-",font=("Arial", 20), foreground='sky blue').grid(row=0,column=0)
        new_label_entry = ttk.Entry(new_centered_frame, font=("Arial", 14), width=30)
        new_label_entry.grid(row=1, column=0, padx=20, pady=10)

        
        confirm_label = tb.Label(new_centered_frame, text="confirmation:", font=("Arial", 20), foreground='sky blue').grid(row=2, column=0)
...
def login():
    ...
    if result:
        ...
        welcome_label = tb.Label(new_window, text="Welcome to the Dashboard", font=("Helvetica", 50, "bold"), background='white', foreground='black')
...
def forgot_password():
    ...
    forgot_label = tb.Label(centered_frame, text="Enter your username to reset password:", font=("Arial", 12), background='white', foreground='blue')
    forgot_label.grid(row=0, column=0, padx=20, pady=20, sticky="nsew")
    
    # Username entry field (styled)
    forgot_username_entry = ttk.Entry(centered_frame, font=("Arial", 14), width=30)
    forgot_username_entry.grid(row=1, column=0, padx=20, pady=10)

    # Label for mobile number entry
    forgot_label = tb.Label(centered_frame, text="Enter your mobile number to reset password:", font=("Arial", 12), background='white', foreground='blue')
    forgot_label.grid(row=2, column=0, padx=20, pady=20, sticky="nsew")
    
    # Mobile number entry field (styled)
    forgot_mobile_entry = ttk.Entry(centered_frame, font=("Arial", 14), width=30)
    ...
    # OTP section
    otp_label = tb.Label(centered_frame, text="Enter the OTP:", font=("Arial", 12), background='white', foreground='blue')
    otp_label.grid(row=5, column=0, padx=20, pady=20, sticky="nsew")
    
    otp_entry = ttk.Entry(centered_frame, font=("Arial", 14), width=30)
...
title_label5 = tb.Label(login_frame, text="Nice to see you! Please log in with your account.", font=("Arial", 24),background='white', foreground='black')
...
user_label = tb.Label(login_frame, text="username:", font=("Arial", 12),background='white', foreground='blue')
...
user_style.configure("userEntry.TFrame", background="white",
                          borderwidth=2, relief="raised", font=("Arial", 14))
user_style.configure("userEntry.TEntry", background="white", font=("Arial", 14), show="*",foreground="grey")  # Use asterisk for masking
...
password_label = tb.Label(login_frame, text="Password:", font=("Arial", 12),background='white', foreground='blue')
...
password_style.configure("PasswordEntry.TFrame", background="white",
                          borderwidth=2, relief="raised", font=("Arial", 14))
password_style.configure("PasswordEntry.TEntry", background="white", font=("Arial", 14), show="*")  # Use asterisk for masking
...
button_style.configure("CustomButton.TButton", font=("Arial", 14),background="lightblue")  # Add background color if desired
